...
namespace Maze
{
    ...
    SubMeshPtr VertexArrayObjectOpenGL::readAsSubMesh() const
    {
        ...
        MAZE_ASSERT(m_indicesType == VertexAttributeType::U16);
        ...
        memcpy(indicesBuffer->getDataRW(), indicesPtr, indicesBuffer->getSize());
                                     ...
                                     attribSemantic = VertexAttributeSemantic((S32)attribSemantic + 1))
        {
            ...
            if (vboData.vbo)
            {
                ...
                memcpy(vertexBuffer->getDataRW(), vertexPtr, vertexBuffer->getSize());
