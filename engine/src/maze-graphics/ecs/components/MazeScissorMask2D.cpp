    ...
    AABB2D const& ScissorMask2D::getScissorBounds()
    {
        if (m_bounds && m_bounds->getTransform())
        {
            if (m_scissorBoundsDirty || m_bounds->isDirty())
            {
                ...
                Vec2F lb = worldTransform.transform(Vec2F(m_paddingLeft, m_paddingBottom));
                Vec2F rb = worldTransform.transform(Vec2F(size.x - m_paddingRight, m_paddingBottom));
                Vec2F rt = worldTransform.transform(Vec2F(size.x - m_paddingRight, size.y - m_paddingTop));
                Vec2F lt = worldTransform.transform(Vec2F(m_paddingLeft, size.y - m_paddingTop));
