...
namespace Maze
{
    ...
    bool MetaPropertyDrawerColorGradient::init(MetaProperty* _metaProperty)
    {
        ...
        m_drawer = PropertyDrawerColorGradient::Create(static_cast<CString>(_metaProperty->getName()));
    ...
    void MetaPropertyDrawerColorGradient::processDataFromUI()
    {
        ...
        ColorGradient gradient = m_drawer->getValue();

        if (MetaClassHelper::IsValueEqual(gradient, m_metaProperty, m_metaInstances))
            return;

        if (m_useEditorActions && EditorToolsActionManager::GetInstancePtr())
            EditorActionHelper::SetValue(gradient, m_metaProperty, m_metaInstances);
        else
            MetaClassHelper::SetValue(gradient, m_metaProperty, m_metaInstances);
    }
