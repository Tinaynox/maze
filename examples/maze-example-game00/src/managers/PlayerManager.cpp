...
namespace Maze
{
    ...
    {
        for (Size i = 0; i < 2; ++i)
            m_players[i] = Player::Create((S32)i);

        return true;
    }

    //////////////////////////////////////////
    PlayerGamepadBindData* PlayerManager::getPlayerBindData(
        S32 _playerIndex,
        PlayerGamepadDeviceType _deviceType,
        S32 _vid,
        S32 _pid)
    {
        PlayerDeviceBindKey key(_playerIndex, _deviceType, _vid, _pid);
        return &m_playerBindData[key];
    }
