...
namespace Maze
{
    ...
    SceneSplash::~SceneSplash()
    {
        ...
        {
            if (Player::GetInstancePtr()->getMainRenderWindow())
            {
                Player::GetInstancePtr()->getMainRenderWindow()->eventRenderTargetResized.unsubscribe(this);
            }
        }
    }
    ...
    {
        RenderWindowPtr const& renderWindow = Player::GetInstancePtr()->getMainRenderWindow();
        if (!EcsRenderScene::init(renderWindow))
            return false;

        Player::GetInstancePtr()->getMainRenderWindow()->eventRenderTargetResized.subscribe(
            this, &SceneSplash::notifyMainRenderWindowResized);

        EntityPtr canvasEntity = createEntity("Canvas");
        m_canvas = canvasEntity->createComponent<Canvas>();        
        m_canvas->setRenderTarget(Player::GetInstancePtr()->getMainRenderWindow());
    ...
    {
        if (!Player::GetInstancePtr()->getRunning())
            return;
    ...
    void SceneSplash::notifyLoadingStep()
    {
        ...
        switch (m_loadingStep)
        {
            ...
            case 16:
            {
                ...
                SceneManager::GetInstancePtr()->loadScene<ScenePlayer>();

                String initialSceneName = PlayerManager::GetInstancePtr()->getPlayerConfig().getString(
                    MAZE_HCS("initialScene"));
